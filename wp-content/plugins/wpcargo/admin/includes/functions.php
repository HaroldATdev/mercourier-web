...
{
    $status = array();
    if (function_exists('wpcargo_set_status') && (did_action('init'))) {
        $status = wpcargo_set_status();
    }
    return apply_filters('wpcargo_default_status', $status);
}

add_action('init', 'wpcargo_set_status', 10);
function wpcargo_set_status()
{
    $status = array(
        __('Pending', 'wpcargo'),
    ...
    return apply_filters('wpcargo_set_status', $status);
}
