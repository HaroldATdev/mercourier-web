        ...
        ?>
            ...
            <div class="postbox" style="max-width:860px">
                <div class="postbox-header">
                    <h2 class="hndle">Tipo de acceso</h2>
                </div>
                <div class="inside">
                    <?php if ($rol_mixto_admin): ?>
                        <div class="notice notice-warning inline">
                            <p>Este usuario tiene roles mezclados (WordPress Admin + WPCargo Admin). Guarda el tipo deseado para normalizar.</p>
                        </div>
                    <?php endif; ?>
                                ...
                                <label style="display:flex;align-items:center;gap:6px;padding:7px 10px;border:1px solid <?php echo $ck ? '#c3d9f4' : '#ddd'; ?>;border-radius:4px;cursor:pointer;background:<?php echo $ck ? '#f0f6fc' : '#fff'; ?>">
                                    ...
                                    <span style="font-size:13px;display:flex;flex-direction:column">
                                        ...
                                        <small style="color:#666;font-size:11px">
                                            slug: <?php echo esc_html($mod['slug']); ?>

                                            <?php if (! empty($mod['sidebar_key'])) : ?>
                                                | key: <?php echo esc_html($mod['sidebar_key']); ?>
                                            <?php endif; ?>

                                            <?php if (! empty($mod['page_id'])) : ?>
                                                | id: <?php echo intval($mod['page_id']); ?>
                                            <?php endif; ?>
                                        </small>
                                    </span>
                                </label>
            ...
            <div class="postbox" style="max-width:860px">
                <div class="postbox-header">
                    <h2 class="hndle">Módulos del dashboard <?php echo $sin_restric ? '<span style="font-weight:400;font-size:12px;color:#00a32a">— Acceso total</span>' : '<span style="font-weight:400;font-size:12px;color:#2271b1">— ' . count($permisos_u ?? []) . ' módulo(s) asignado(s)</span>'; ?></h2>
                </div>
                <div class="inside">
                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
        ...
        <?php else: ?>
            ...
            <table class="wp-list-table widefat fixed striped" style="max-width:1000px">
                ...
                <tbody>
                    ...
                    ?>
                        ...
                        <tr>
                            <td colspan="6" style="text-align:center;padding:20px;color:#888">No hay usuarios.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
            <div style="margin-top:16px;padding:10px 14px;background:#fff8e5;border:1px solid #f0c040;border-radius:4px;max-width:1000px">
                <p style="margin:0;font-size:13px"><span class="dashicons dashicons-info-outline" style="vertical-align:middle;color:#b45309"></span>
                    <strong>WPCargo Admin</strong> no puede acceder a wp-admin. Al loguearse, es redirigido directamente al dashboard de WPCargo.
                </p>
            </div>
        <?php endif; ?>
    </div>