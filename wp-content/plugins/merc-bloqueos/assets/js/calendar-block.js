jQuery(document).ready(function($) {
    ...
    // Si el formulario tiene un select de tipo, usarlo como fuente de verdad al cambiar
    function getTipoEnvioActual() {
        var selForm = $('select[name="wpcargo_type_of_shipment"], select[name="tipo_envio"]').first();
        if (selForm.length && selForm.val()) {
            var v = selForm.val().toLowerCase().trim();
            if (v === 'express' || v.indexOf('agencia') !== -1 || v.indexOf('express') !== -1) return 'express';
            if (v === 'normal' || v.indexOf('emprendedor') !== -1 || v.indexOf('normal') !== -1) return 'normal';
            return v;
        }
        return tipoEnvio;
    }
    ...
    /**
     * Carga las reglas de bloqueo para un tipo de servicio y las aplica al calendario.
     * Se reutiliza cuando el usuario cambia el tipo de envío dentro del formulario.
     */
    function cargarYAplicarBloqueo(tipo) {
        $.post(mercBloqueos.ajax_url, {
            ...
            var data = res.data;
            console.log('[MERC] Bloqueo Info recibido para tipo=' + tipo + ':', data);
            ...
            // ── Aviso de servicio específico bloqueado (sin ocultar el form) ────────
            // Si es_formulario_bloqueado, la fecha de hoy estará bloqueada pero el
            // formulario sigue disponible para programar para mañana.
            var avisoServId = 'merc-aviso-serv-' + tipo;
            $('#' + avisoServId).remove();
            if (data.is_formulario_bloqueado) {
                var labelTipo = (tipo === 'express') ? 'Agencia (Express)' : 'Emprendedor (Normal)';
                $('<div id="' + avisoServId + '" class="alert alert-warning" style="margin-top:10px"></div>')
                    .html('<i class="fa fa-clock-o mr-1"></i><strong>Servicio ' + labelTipo + ' bloqueado para hoy.</strong> La próxima fecha disponible será asignada automáticamente.')
                    .insertBefore('#wpcfe-add-shipment-form');
            }
                ...
                inputs.each(function() {
                    ...
                    if (fp) {
                        ...
                        if (isAdmin) {
                            ...
                            // Cliente: aplicar restricciones y forzar próxima fecha disponible
    ...
    } // fin cargarYAplicarBloqueo

    // Carga inicial con el tipo del URL
    cargarYAplicarBloqueo(tipoEnvio);

    // Si el formulario tiene un select de tipo de envío, re-evaluar al cambiar
    $(document).on('change', 'select[name="wpcargo_type_of_shipment"], select[name="tipo_envio"]', function() {
        var nuevoTipo = getTipoEnvioActual();
        console.log('[MERC] Tipo de envío cambiado a:', nuevoTipo);
        $('[id^="merc-aviso-serv-"]').remove();
        $('#merc-aviso-fecha-cambiada').remove();
        cargarYAplicarBloqueo(nuevoTipo);
    });
});
