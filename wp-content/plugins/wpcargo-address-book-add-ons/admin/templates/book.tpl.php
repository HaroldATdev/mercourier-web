<?php 
    $default_user   = get_user_meta(get_current_user_id(), 'default_'.$book, true );
?>
<h1><?php esc_html_e('My Address Book', 'wpcargo-address-book' ); ?> <a id="add-address" href="#" class="page-title-action" data-book="<?php echo $book; ?>"><?php esc_html_e('Add', 'wpcargo-address-book' ); ?> <?php echo ucfirst($book); ?> <?php esc_html_e('Address', 'wpcargo-address-book' ); ?></a></h1>
<div id="address-book-menu">
	<h2 class="nav-tab-wrapper">
		<?php
		if( !get_option('wpc_disable_address_receiver_search') ){
			?><a href="<?php echo admin_url('admin.php?page=my-address-book'); ?>&book=receiver" class="nav-tab<?php echo ( $book == 'receiver' ) ? ' nav-tab-active' : ''; ?>"><?php esc_html_e('Receiver Book', 'wpcargo-address-book' ); ?></a><?php
		}
		if( !get_option('wpc_disable_address_shipper_search') ){
			?><a href="<?php echo admin_url('admin.php?page=my-address-book'); ?>&book=shipper" class="nav-tab<?php echo ( $book == 'shipper' ) ? ' nav-tab-active' : ''; ?>"><?php esc_html_e('Shipper Book', 'wpcargo-address-book' ); ?></a><?php
		}
		?>
	</h2>
</div>
<form style="float: right;  margin-bottom: 18px;" action="<?php echo admin_url('admin.php'); ?>" method="get">
	<input type="hidden" name="page" value="my-address-book"/>
				...
				<?php if( in_array('administrator', $current_user->roles) ) : ?>
					<th> 
						<?php if( $book_address->have_posts() ) : ?>
							<input id="public-bulk" type="checkbox" book="<?php echo $book; ?>"> 
						<?php endif; ?>
						<?php echo esc_html__('Public', 'wpcargo-address-book') ?> 
					</th>
				<?php endif; ?>
				<th>&nbsp;</th>
				<th>&nbsp;</th>
		...
		<?php if ( $book_address->have_posts() ) : ?>
			<?php while ( $book_address->have_posts() ) : $book_address->the_post(); ?>
				...
				<tr id="address-<?php echo get_the_ID(); ?>" class="view-mode">
					...
					<td>                     
						<input class="public-user" type="checkbox" name="public-user[]" value="<?php echo get_the_ID(); ?>" book="<?php echo $book; ?>" <?php echo ( $is_public == 1 )? 'checked' : ''; ?>>
					</td>
					<td class="update">
						<span class="update-button">
							<a href="#" class="edit-address button button-quotation" public-id="<?php echo $is_public; ?>" default-id="<?php echo $default_user; ?>" data-id="<?php echo get_the_ID(); ?>" data-book="<?php echo get_post_meta( get_the_ID(), 'book', TRUE ); ?>"><?php esc_html_e('Update', 'wpcargo-address-book' ); ?></a>
						</span>
					</td>
	...
	<div class="modal-content">
		...
		<div d = "add-address-wrapper" class="modal-body">
			<form id="add-address-book-form">                
                ...
                <input class="default_address" type="checkbox" value="1" name="<?php echo 'default_'.$book; ?>"  ><span><?php echo esc_html__('Default', 'wpcargo-address-book') ?></span> 
                <input class="public_address" type="checkbox" value="1" name="<?php echo 'public_'.$book; ?>"  ><span><?php echo esc_html__('Public', 'wpcargo-address-book') ?></span> 
